Public Class frmSiparisRaporu
    ...
    Private Sub mnuSetBoyaHazirlik_Click(sender As Object, e As EventArgs) Handles mnuSetBoyaHazirlik.Click
        Try
            If GridView1.FocusedRowHandle < 0 Then Exit Sub
            Dim sSipDetayID As String = "-99999"
            For i As Integer = 0 To GridView1.GetSelectedRows.Length - 1
            ...
            Dim BoyahaneID As Integer = 0
            Dim Durumu As String = IfNull(GridView1.GetDataRow(GridView1.FocusedRowHandle)("Durumu"), "")  ' IfNull(MasterLink.CurrentRow("Durumu"), "")
            If Durumu <> "BOYA-HAZIRLIK" Then
                If MsgBox(GridView1.GetSelectedRows.Length.ToString() & " adet siparişin durumunu 'BOYA-HAZIRLIK' olarak değiştirmek istediğinizden emin misiniz?", MsgBoxStyle.Question + MsgBoxStyle.YesNoCancel, "Durumu Değiştir?") <> MsgBoxResult.Yes Then Exit Sub
            End If
            'Dim f As New frmBoyahaneler
            'f.FormSelect()
            'If f.DialogResult = DialogResult.OK Then
            ...
            Dim cmd As New SqlClient.SqlCommand
            cmd.Connection = AppConn.dbConn
            cmd.CommandText = "UPDATE SipDetay SET Durumu='BOYA-HAZIRLIK', BoyahaneID=" & BoyahaneID & " , ModifiedBy=" & AppConn.ActiveUser.UserID & ",ModifiedDate=GETDATE() WHERE Deleted = 0 AND SipDetayID IN (" & sSipDetayID & ") "
            cmd.ExecuteNonQuery()
