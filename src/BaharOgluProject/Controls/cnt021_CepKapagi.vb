Public Class cnt021_CepKapagi
    Dim bNoAction As Boolean = False
    Dim YakaCizgiRKafa As String = ""

    Private Sub cnt021_CepKapagi_Initialize_Control(ByVal sender As System.Windows.Forms.Control, ByVal e As System.EventArgs) Handles Me.Initialize_Control
        Try
    ...
    Private Sub cnt021_CepKapagi_Initialize_Control_ForAppMode(ByVal sender As System.Windows.Forms.Control, ByVal e As System.EventArgs) Handles Me.Initialize_Control_ForAppMode
        Try
            ...
            Dim GogusMansetID As Long = 0

            If NeoTable2.Rows.Count > 0 Then
                GogusMansetID = NeoTable2.Rows(0).Item("nID")
            End If
            NeoTable3.Connection = Me.SqlConn
            NeoTable3.SelectCommand.CommandText &= " AND GogusMansetID>0 AND GogusMansetID=" & GogusMansetID & " "
            OpenTable(NeoTable3)
    ...
    Private Sub cnt021_CepKapagi_IsOkay(ByVal sender As System.Windows.Forms.Control, ByRef e As IsOkayEventArgs) Handles Me.IsOkay
        Try
            ...
            If NeoTable2.Rows.Count > 0 Then
                If If0Null(NeoTable2.Rows(0).Item("RenkID")) <= 0 Then
                    smsg &= "[CEP KAPAÄI] Rengi seÃ§ilmemis!" & vbNewLine
                    bCancel = True
                End If
                If If0Null(NeoTable2.Rows(0).Item("CizgiRKafaLakosID")) <= 0 Then
                    smsg &= "[CEP KAPAÄI] Ã‡izgi seÃ§ilmemis!" & vbNewLine
                    bCancel = True
                End If
            ...
            For i As Integer = 0 To NeoTable3.Rows.Count - 1
                ...
                If dr.RowState <> DataRowState.Deleted And dr.RowState <> DataRowState.Detached Then
                    If If0Null(dr("RenkID")) <= 0 Then
                        smsg &= "[CEP KAPAÄI] #" & i + 1 & " satirda renk seÃ§ilmemis!" & vbNewLine
                        bCancel = True
                    End If
    ...
    Private Sub cnt021_CepKapagi_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
    ...
    Private Sub cnt021_CepKapagi_Save_Control(ByVal sender As System.Windows.Forms.Control, ByRef e As Module1.SaveEventArgs) Handles Me.Save_Control
        Try
            ...
            Else
                ...
                If NeoTable2.Rows.Count > 0 Then
                    For Each dr As DataRow In NeoTable3.Rows
                        If dr.RowState <> DataRowState.Deleted And dr.RowState <> DataRowState.Detached Then
                            dr("GogusMansetID") = If0Null(NeoTable2.Rows(0).Item("nID"))
                        End If
                    Next
            ...
            If ApplicationMode = True Then
                If (If0Null(MasterTable.Rows(0).Item("Secmeli")) = 1 And If0Null(MasterTable2.Rows(0).Item("bVar")) = 1) Or If0Null(MasterTable.Rows(0).Item("Secmeli")) = 0 Then
                    ...
                    End If
                    Aciklamalar.Add("CEP KAPAÄI Gramaj :" & IpGramaj & " " & sGramajArtis & "    T.Gramaj:" & TIpGramaj & " ")

                    Aciklamalar.Add("CEP KAPAÄI Adedi :" & ToplamAdet & " Tarak ")
                    '---------------------
                    ...
                    UretimBilgiYaz(SqlConn.SqlConnection, SipDetayID, "021", "CEP KAPAÄI", Aciklamalar, AnaRenkTam)
                    SiparisBilgiYaz(SqlConn.SqlConnection, SipDetayID, "021", "CEP KAPAÄI", Aciklamalar2, AnaRenkTam)
                End If
            End If
            '-----------------------------
        Catch ex As Exception
