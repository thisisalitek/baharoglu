Public Class cnt007_DubleYaka
    Dim bNoAction As Boolean = False
    Dim DubleYakaCizgiRKafa As String = ""

    Private Sub cnt007_DubleYaka_Initialize_Control(ByVal sender As System.Windows.Forms.Control, ByVal e As System.EventArgs) Handles Me.Initialize_Control
        Try
    ...
    Private Sub cnt007_DubleYaka_Initialize_Control_ForAppMode(ByVal sender As System.Windows.Forms.Control, ByVal e As System.EventArgs) Handles Me.Initialize_Control_ForAppMode
        Try
    ...
    Private Sub cnt007_DubleYaka_IsOkay(ByVal sender As System.Windows.Forms.Control, ByRef e As IsOkayEventArgs) Handles Me.IsOkay
        Try
            ...
            If MasterTable2.Rows.Count > 0 Then
                If If0Null(MasterTable2.Rows(0).Item("RenkID")) <= 0 Then
                    smsg &= "[DUBLE YAKA] Duble yaka rengi se̤ilmemis!" & vbNewLine
                    bCancel = True
                End If
            End If
            For i As Integer = 0 To NeoTable2.Rows.Count - 1
                ...
                If dr.RowState <> DataRowState.Deleted And dr.RowState <> DataRowState.Detached Then
                    If If0Null(dr("RenkID")) <= 0 Then
                        smsg &= "[DUBLE YAKA] #" & i + 1 & " satirda renk se̤ilmemis!" & vbNewLine
                        bCancel = True
                    End If
                    If Trim(IfNull(dr("RKafa"), "")) = "" Then
                        smsg &= "[DUBLE YAKA] #" & i + 1 & " satirda ̤izgi sayisi hatali!" & vbNewLine
                        bCancel = True
                    End If
                End If
            Next
        ...
        End Try
    End Sub
    Private Sub cnt007_DubleYaka_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
    ...
    Private Sub cnt007_DubleYaka_Save_Control(ByVal sender As System.Windows.Forms.Control, ByRef e As Module1.SaveEventArgs) Handles Me.Save_Control
        Try
            ...
            If ApplicationMode = True Then
                If (If0Null(MasterTable.Rows(0).Item("Secmeli")) = 1 And If0Null(MasterTable2.Rows(0).Item("bVar")) = 1) Or If0Null(MasterTable.Rows(0).Item("Secmeli")) = 0 Then
                    ...
                    If MasterTable2.Rows.Count > 0 Then
                        ...
                        sSipBuf = "DUBLE YAKA RENK:" & GetRenkKodIsim(SqlConn.SqlConnection, If0Null(MasterTable2.Rows(0).Item("RenkID")))
                        ...
                        End If
                        Aciklamalar.Add("DUBLE YAKA TUR :" & If0Null(MasterTable.Rows(0).Item("Tur")) & "    T.Tur:" & TTur)
                    ...
                    UretimBilgiYaz(SqlConn.SqlConnection, SipDetayID, "007", "DUBLE YAKA", Aciklamalar, AnaRenk)
                    SiparisBilgiYaz(SqlConn.SqlConnection, SipDetayID, "007", "DUBLE YAKA", Aciklamalar2, AnaRenk)
                End If
            End If
            '-----------------------------
...
Basa:
                    ...
                    For Each dr As DataRow In NeoTable2.Rows
                        ...
                        End If
                    Next
                    DubleYakaCizgiRKafa = IfNull(.ReturnDataRow("Aciklama"), "")
                End If
            End With
        Catch ex As Exception
            MsgBox(ex.Message)
