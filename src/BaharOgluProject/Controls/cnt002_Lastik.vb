Public Class cnt002_Lastik
    Dim bNoAction As Boolean = False
    Dim BelCizgiRKafa As String = ""
    Dim KolCizgiRKafa As String = ""

    Private Sub cnt0002_Lastik_Initialize_Control(ByVal sender As System.Windows.Forms.Control, ByVal e As System.EventArgs) Handles Me.Initialize_Control
        Try
    ...
    Private Sub cnt002_Lastik_Initialize_Control_ForAppMode(ByVal sender As System.Windows.Forms.Control, ByVal e As System.EventArgs) Handles Me.Initialize_Control_ForAppMode
        Try
    ...
    Private Sub cnt002_Lastik_IsOkay(ByVal sender As System.Windows.Forms.Control, ByRef e As IsOkayEventArgs) Handles Me.IsOkay
        Try
            ...
            If MasterTable2.Rows.Count > 0 Then
                If If0Null(MasterTable2.Rows(0).Item("RenkID")) <= 0 Then
                    smsg &= "[LASTIK] Lastik bel rengi seçilmemiş!" & vbNewLine
                    bCancel = True
                End If
                ...
                For i As Integer = 0 To NeoTable2.Rows.Count - 1
                    ...
                    If dr.RowState <> DataRowState.Deleted And dr.RowState <> DataRowState.Detached Then
                        ...
                        If If0Null(dr("RenkID")) <= 0 Then
                            smsg &= "[LASTIK] belde #" & i + 1 & " satırda renk seçilmemiş!" & vbNewLine
                            bCancel = True
                        End If

                        If Trim(IfNull(dr("RKafa"), "")) = "" Then
                            smsg &= "[LASTIK] belde #" & i + 1 & " satırda çizgi sayısı hatalı!" & vbNewLine
                            bCancel = True
                        End If
                    End If
                Next
                ...
                If If0Null(MasterTable2.Rows(0).Item("IntParam1")) = 1 Then
                    ...
                    If If0Null(MasterTable2.Rows(0).Item("RenkID2")) <= 0 Then
                        smsg &= "[LASTIK] Lastik kol rengi seçilmemiş!" & vbNewLine
                        bCancel = True
                    End If
                    ...
                    For i As Integer = 0 To NeoTable3.Rows.Count - 1
                        ...
                        If dr.RowState <> DataRowState.Deleted And dr.RowState <> DataRowState.Detached Then
                            If If0Null(dr("RenkID")) <= 0 Then
                                smsg &= "[LASTIK] kolda #" & i + 1 & " satırda renk seçilmemiş!" & vbNewLine
                                bCancel = True
                            End If
                            If Trim(IfNull(dr("RKafa"), "")) = "" Then
                                smsg &= "[LASTIK] kolda #" & i + 1 & " satırda çizgi sayısı hatalı!" & vbNewLine
                                bCancel = True
                            End If
                        End If
                    Next
    ...
    Private Sub cnt0002_Lastik_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
    ...
    Private Sub cnt0002_Lastik_Save_Control(ByVal sender As System.Windows.Forms.Control, ByRef e As Module1.SaveEventArgs) Handles Me.Save_Control
        Try
            ...
            Else
                ...
                Else
                    ...
                    cmd.ExecuteNonQuery()
                    cmd.CommandText = "DELETE FROM SipDetayParam_002_Lastik_Kol WHERE SipDetayParamID=" & SipDetayParamID
                    cmd.ExecuteNonQuery()
                End If
            ...
            If ApplicationMode = True Then
                If (If0Null(MasterTable.Rows(0).Item("Secmeli")) = 1 And If0Null(MasterTable2.Rows(0).Item("bVar")) = 1) Or If0Null(MasterTable.Rows(0).Item("Secmeli")) = 0 Then
                    ...
                    If RadioGroup1.SelectedIndex = 0 Then
                        Aciklamalar.Add("BEL+KOL Tek Parça")
                    Else
                        Aciklamalar.Add("BEL ve KOL  Ayrı 2 Parça")
                    End If

                    If MasterTable2.Rows.Count > 0 Then

                        If RadioGroup1.SelectedIndex = 0 Then
                            sbuf = "LASTIK BEL+KOL RENK:" & GetRenkKodIsim(SqlConn.SqlConnection, If0Null(MasterTable2.Rows(0).Item("RenkID")))
                        Else
                            sbuf = "LASTIK BEL RENK:" & GetRenkKodIsim(SqlConn.SqlConnection, If0Null(MasterTable2.Rows(0).Item("RenkID")))
                        End If
                        ...
                        sSipBuf = "LASTİK RENK:" & GetRenkKodIsim(SqlConn.SqlConnection, If0Null(MasterTable2.Rows(0).Item("RenkID")))
                        Aciklamalar2.Add(sSipBuf)
                        If If0Null(MasterTable2.Rows(0).Item("CizgiSayisi")) <= 0 Then
                            sSipBuf = "LASTİK ÇİZGİ :YOK "
                        Else
                            sSipBuf = "LASTİK ÇİZGİ :" & If0Null(MasterTable2.Rows(0).Item("CizgiSayisi")) & "   RENKLER:"
                        ...
                        If If0Null(MasterTable2.Rows(0).Item("CizgiSayisi")) > 0 Then
                            If Trim(BelCizgiRKafa) <> "" Then
                                sSipBuf = "LASTİK ÇİZGİ TÜR :" & BelCizgiRKafa
                                Aciklamalar2.Add(sSipBuf)
                            End If
                        End If
                        ...
                        If If0Null(MasterTable2.Rows(0).Item("IntParam1")) > 0 Then
                            ...
                            sbuf = "LASTIK KOL RENK:" & GetRenkKodIsim(SqlConn.SqlConnection, If0Null(MasterTable2.Rows(0).Item("RenkID2")))

                            If If0Null(MasterTable2.Rows(0).Item("CizgiSayisi2")) <= 0 Then
                                sbuf &= "   ÇİZGİ :YOK "
                            Else
                                sbuf &= "   ÇİZGİ :" & If0Null(MasterTable2.Rows(0).Item("CizgiSayisi2"))
                            End If
                            AciklamalarKol.Add(sbuf)

                            '------------
                            sSipBuf = "LASTİK KOL RENK:" & GetRenkKodIsim(SqlConn.SqlConnection, If0Null(MasterTable2.Rows(0).Item("RenkID2")))
                            Aciklamalar2Kol.Add(sSipBuf)
                            If If0Null(MasterTable2.Rows(0).Item("CizgiSayisi2")) <= 0 Then
                                sSipBuf = "LASTİK KOL ÇİZGİ :YOK "
                            Else
                                sSipBuf = "LASTİK KOL ÇİZGİ :" & If0Null(MasterTable2.Rows(0).Item("CizgiSayisi2")) & "   RENKLER:"
                    ...
                    UretimBilgiYaz(SqlConn.SqlConnection, SipDetayID, "002", "LASTIK", Aciklamalar, AnaRenk)
                    SiparisBilgiYaz(SqlConn.SqlConnection, SipDetayID, "002", "LASTIK", Aciklamalar2, AnaRenk)
                    If AciklamalarKol.Count > 0 Then

                        UretimBilgiYaz(SqlConn.SqlConnection, SipDetayID, "002", "LASTIK", AciklamalarKol, AnaRenkKol, False)
                        SiparisBilgiYaz(SqlConn.SqlConnection, SipDetayID, "002", "LASTIK", Aciklamalar2Kol, AnaRenkKol, False)
