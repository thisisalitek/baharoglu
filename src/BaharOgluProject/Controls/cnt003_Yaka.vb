Public Class cnt003_Yaka
    ...
    Private Sub cnt003_Yaka_Initialize_Control(ByVal sender As System.Windows.Forms.Control, ByVal e As System.EventArgs) Handles Me.Initialize_Control
        Try
    ...
    Private Sub cnt003_Yaka_Initialize_Control_ForAppMode(ByVal sender As System.Windows.Forms.Control, ByVal e As System.EventArgs) Handles Me.Initialize_Control_ForAppMode
        Try
    ...
    Private Sub cnt003_Yaka_IsOkay(ByVal sender As System.Windows.Forms.Control, ByRef e As IsOkayEventArgs) Handles Me.IsOkay
        Try
            ...
            If MasterTable2.Rows.Count > 0 Then
                If If0Null(MasterTable2.Rows(0).Item("RenkID")) <= 0 Then
                    smsg &= "[YAKA] Yaka rengi seÃ§ilmemis!" & vbNewLine
                    bCancel = True
                End If
            End If
            For i As Integer = 0 To NeoTable2.Rows.Count - 1
                ...
                If dr.RowState <> DataRowState.Deleted And dr.RowState <> DataRowState.Detached Then
                    If If0Null(dr("RenkID")) <= 0 Then
                        smsg &= "[YAKA] #" & i + 1 & " satirda renk seÃ§ilmemis!" & vbNewLine
                        bCancel = True
                    End If
                    If Trim(IfNull(dr("RKafa"), "")) = "" Then
                        smsg &= "[YAKA] #" & i + 1 & " satirda Ã§izgi sayisi hatali!" & vbNewLine
                        bCancel = True
                    End If
                End If
            Next
        ...
        End Try
    End Sub
    Private Sub cnt003_Yaka_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
    ...
    Private Sub cnt003_Yaka_Save_Control(ByVal sender As System.Windows.Forms.Control, ByRef e As Module1.SaveEventArgs) Handles Me.Save_Control
        Try
            ...
            If ApplicationMode = True Then
                If (If0Null(MasterTable.Rows(0).Item("Secmeli")) = 1 And If0Null(MasterTable2.Rows(0).Item("bVar")) = 1) Or If0Null(MasterTable.Rows(0).Item("Secmeli")) = 0 Then
                    ...
                    If MasterTable2.Rows.Count > 0 Then
                        ...
                        sSipBuf = "YAKA RENK:" & GetRenkKodIsim(SqlConn.SqlConnection, If0Null(MasterTable2.Rows(0).Item("RenkID")))
                        Aciklamalar2.Add(sSipBuf)

                        If If0Null(MasterTable2.Rows(0).Item("CizgiSayisi")) <= 0 Then
                            sSipBuf = "YAKA Ã‡Ä°ZGÄ° :YOK "
                        Else
                            sSipBuf = "YAKA Ã‡Ä°ZGÄ° :" & If0Null(MasterTable2.Rows(0).Item("CizgiSayisi")) & "   RENKLER:"
                        ...
                        Aciklamalar.Add("YAKA TUR :" & If0Null(MasterTable.Rows(0).Item("Tur")) & "    T.Tur:" & TTur)

                        Aciklamalar2.Add(sSipBuf)
                        If If0Null(MasterTable2.Rows(0).Item("CizgiSayisi")) > 0 Then
                            If Trim(YakaCizgiRKafa) <> "" Then
                                sSipBuf = "YAKA Ã‡Ä°ZGÄ° TUR :" & YakaCizgiRKafa
                                Aciklamalar2.Add(sSipBuf)
                            End If
                        End If
                    End If
                    'Aciklamalar.Add("------------------------------")
                    UretimBilgiYaz(SqlConn.SqlConnection, SipDetayID, "003", "YAKA", Aciklamalar, AnaRenk)
                    SiparisBilgiYaz(SqlConn.SqlConnection, SipDetayID, "003", "YAKA", Aciklamalar2, AnaRenk)
                End If
            End If
            '-----------------------------
        Catch ex As Exception
...
Basa:
                    ...
                    YakaCizgiRKafa = IfNull(.ReturnDataRow("Aciklama"), "")
                End If
            End With
        Catch ex As Exception
            MsgBox(ex.Message)
